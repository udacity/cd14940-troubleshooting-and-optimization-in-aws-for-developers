...
PLANTED ISSUE: When publishing to SNS, the order-service does NOT include
the 'eventType' message attribute. The SNS subscription filter policy
expects this attribute, causing messages to be filtered out.
"""
...
def lambda_handler(event, context):
    ...
    PLANTED ISSUE: This handler expects messages with 'eventType' attribute
    for filtering, but the order-service doesn't send this attribute.
    """
    ...
    for record in event.get('Records', []):
        try:
            ...
            # PLANTED ISSUE: This attribute is never set by the order-service
