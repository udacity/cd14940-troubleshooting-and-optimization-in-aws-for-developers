...
Globals:
  Function:
    Runtime: python3.11
    Timeout: 3  # PLANTED ISSUE: Too short for DynamoDB scan
    MemorySize: 128
...
def handler(event, context):
    """Notification handler - PLANTED ISSUE: Missing eventType message attribute"""
    ...
    for record in event.get('Records', []):
        try:
            ...
            # PLANTED ISSUE: Publishing without required MessageAttributes
            # The downstream subscriber filters on eventType but it's not included
            sns.publish(
                TopicArn=TOPIC_ARN,
                Message=json.dumps({
                    'notification': body,
